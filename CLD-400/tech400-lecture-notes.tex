...
Documents and queries are represented as binary term-incidence vectors (as in Week 1's incidence matrix). ``Independence'' means terms are assumed to occur \emph{independently of one another} within the relevant documents, and independently within the non-relevant documents (a strong simplifying assumption, but one that leads to a tractable, useful ranking formula).
\end{defbox}

For a single query term $t$, define, over the whole judged collection: $N$ = total documents, $R$ = total relevant documents, $n_t$ = documents containing $t$, $r_t$ = relevant documents containing $t$. This gives a $2\times2$ contingency table:
...
\midrule
$t$ present & $r_t$ & $n_t - r_t$ \\
$t$ absent  & $R - r_t$ & $(N - n_t) - (R - r_t)$ \\
\bottomrule
\end{tabular}
\end{center}

\begin{formulabox}[Key Formula 5.1 -- RSJ (Robertson--Sparck Jones) Term Weight]
Let $p_t = r_t / R$ (probability a relevant document contains $t$) and $u_t = (n_t - r_t)/(N-R)$ (probability a non-relevant document contains $t$). The retrieval status value contributed by matching term $t$ is:
\[
c_t = \log \frac{p_t(1-u_t)}{u_t(1-p_t)}
\]
A document's overall score is the sum of $c_t$ over all query terms $t$ that it contains. Larger $c_t$ means $t$ is a much better discriminator of relevance (much more likely in relevant than non-relevant documents).
\end{formulabox}
...
\[
p_t = \frac{r_t}{R} = \frac{20}{25} = 0.8, \qquad u_t = \frac{n_t-r_t}{N-R} = \frac{50-20}{1000-25} = \frac{30}{975} \approx 0.03077
\]
\[
c_t = \log_2 \frac{0.8\times(1-0.03077)}{0.03077\times(1-0.8)} = \log_2\frac{0.8\times0.96923}{0.03077\times0.2} = \log_2\frac{0.77538}{0.006154}
\]
\[
= \log_2(126.0) \approx 6.977 \text{ bits}
\]
...
\begin{defbox}[Definition 5.4 -- Query Likelihood Model]
For each document $d$, build a \textbf{unigram language model} $M_d$: a probability distribution over vocabulary terms estimated from $d$'s own text. Rank documents by $P(q \mid M_d)$ -- the probability that $d$'s language model would have generated the query $q$. Intuitively: \emph{which document ``sounds like'' it was written by someone who would also write this query?}
\end{defbox}

\begin{formulabox}[Key Formula 5.2 -- MLE Unigram Model and the Zero-Probability Problem]
\[
P_{\text{mle}}(t \mid d) = \frac{tf_{t,d}}{|d|}
\]
The raw maximum-likelihood estimate assigns probability \textbf{exactly zero} to any query term absent from $d$ -- a single missing term would make the entire product $P(q\mid d)=\prod_t P(t\mid d)$ zero, however well $d$ matches the rest of the query. This is why language models for IR always \textbf{smooth} the estimate with a collection-wide background model.
\end{formulabox}

\subsection{Jelinek--Mercer Smoothing}

\begin{formulabox}[Key Formula 5.3 -- Jelinek--Mercer (Linear Interpolation) Smoothing]
\[
P_{JM}(t\mid d) = (1-\lambda)\,P_{\text{mle}}(t\mid d) \;+\; \lambda\, P(t\mid C)
\]
where $P(t\mid C)$ is the term's relative frequency in the whole collection $C$ (the background model) and $\lambda\in[0,1]$ controls the smoothing strength. The query likelihood is $P(q\mid d) = \prod_{t\in q} P_{JM}(t\mid d)$.
\end{formulabox}
...
\begin{formulabox}[Key Formula 5.4 -- Dirichlet-Prior Smoothing]
\[
P_{\text{Dir}}(t\mid d) = \frac{tf_{t,d} + \mu\, P(t\mid C)}{|d| + \mu}
\]
where $\mu$ is a single free parameter (behaving like a pseudo-count of ``extra'' collection-flavored word occurrences added to every document). Unlike Jelinek--Mercer's fixed $\lambda$, Dirichlet smoothing automatically smooths short documents more heavily than long ones, since $\mu$ is a constant while $|d|$ varies.
\end{formulabox}
